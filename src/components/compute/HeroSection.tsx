...
export function HeroSection() {
  return (
    <section className="relative min-h-[90vh] flex items-center justify-center bg-navy-gradient overflow-hidden hero-overlay">
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <Badge className="mb-6 bg-gradient-to-r from-cyan-400 to-green-400 text-white border-0">
              {computePageData.hero.badge}
            </Badge>
            
            <h1 className="text-4xl lg:text-6xl font-bold text-white mb-6 leading-tight">
              {computePageData.hero.title}{' '}
              <span className="bg-gradient-to-r from-cyan-400 to-green-400 bg-clip-text text-transparent">
                {computePageData.hero.highlightText}
              </span>{' '}
              {computePageData.hero.titleEnd}
            </h1>
            
            <p className="text-xl text-gray-200 mb-8 leading-relaxed">
              {computePageData.hero.description}
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
              <motion.button 
                ...
                whileTap={{ scale: 0.95 }}
              >
                {computePageData.hero.primaryCTA}
              </motion.button>
              <motion.button 
                ...
                whileTap={{ scale: 0.95 }}
              >
                {computePageData.hero.secondaryCTA}
              </motion.button>
            </div>
          </motion.div>
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <div className="font-semibold text-slate-900">Server-{i + 1}</div>
                  <div className="text-sm text-slate-500">Intel Xeon Gold</div>
                </div>
              </div>
            ...
            <div className="mt-4 grid grid-cols-3 gap-4 text-sm text-slate-500">
              <div>CPU: {85 + i * 5}%</div>
              <div>RAM: {8 + i * 4}GB</div>
              <div>Storage: {128 + i * 64}GB</div>
            </div>
          </motion.div>
        ))}
      </div>
