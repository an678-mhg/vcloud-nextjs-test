        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <Badge className="mb-6 bg-gradient-to-r from-cyan-400 to-green-400 text-white border-0">
              {kubernetesPageData.hero.badge}
            </Badge>
            
            <h1 className="text-4xl lg:text-6xl font-bold text-white mb-6 leading-tight">
              {kubernetesPageData.hero.title}{' '}
              <span className="bg-gradient-to-r from-cyan-400 to-green-400 bg-clip-text text-transparent">
                {kubernetesPageData.hero.highlightText}
              </span>{' '}
              {kubernetesPageData.hero.titleEnd}
            </h1>
            
            <p className="text-xl text-gray-200 mb-8 leading-relaxed">
              {kubernetesPageData.hero.description}
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
              ...
              <motion.button 
                ...
                whileTap={{ scale: 0.95 }}
              >
                {kubernetesPageData.hero.secondaryCTA}
              </motion.button>
            </div>
          </motion.div>
...
// Kubernetes Visualization Component
function KubernetesVisualization() {
  return (
    <div className="relative">
      {/* Main pods/containers */}
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <div className="font-semibold text-slate-900">Pod-{i + 1}</div>
            ...
            <div className="mt-4 grid grid-cols-3 gap-4 text-sm text-slate-500">
              <div>CPU: {30 + i * 20}%</div>
              <div>Memory: {40 + i * 15}%</div>
              <div>Replicas: {2 + i}</div>
            </div>
          </motion.div>
        ))}
      </div>
